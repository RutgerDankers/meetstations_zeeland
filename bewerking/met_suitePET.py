...
from math import sin,cos,atan,pi,exp,nan,log,sqrt

#*************************************************************************************
# Block 1:
# Solar radiation functions for first-order estimates in practical applications

def fr_diffuse(solar_down,lat:float=52.,lon:float=5.1,DOY:int=180,utc_dec:float=11.66):
    """Estimate fraction of diffuse solar radiation according to Erbs et al. (1982) """
    
    trans=transmissivity(solar_down,lat,lon,DOY,utc_dec)
    if trans <= 0.22:
        frdif = 1.-0.09*trans
    elif trans <=0.80:
...
def transmissivity(solar_down,lat=52.,lon=5.1,DOY=180,utc_dec=11.66):
    """Estimate atmospheric transmissivity according to De Rooy and Holtslag (1999) """
    
    return solar_down/(1367.*sin_solar_elev(lat,lon,DOY,utc_dec))


def sin_solar_elev(lat:float=52.,lon:float=5.1,DOY:int=171,utc_dec:float=11.66,):
    """
       Sine of solar elevation as well as cosine of zenith angle for a given 
       location (lat,lon) and time (UTC)
       Note: default De Bilt (Netherlands), 11.66 UTC on day-of-year 171      
    """
    
    fac1 = sin(solar_decl(DOY))*sin(pi*lat/180.)
    fac2 = cos(hour_angle(lon,utc_dec))*cos(solar_decl(DOY))*cos(pi*lat/180)
    return (fac1+fac2)


def solar_decl(DOY:int=180,):
    """Solar declination on a given day-of-year."""
    return 0.409*cos(2.*pi*(DOY-171)/365.25)


def hour_angle(lon:float=5.1,utc_dec:float=11.66,):
    """Approximate solar hour angle in radians"""
    return pi*(utc_dec/12. + lon/180. - 1.)

def solar_clear(lon:float=5.1,lat:float=52.,DOY:int=171,utc_dec:float=11.66,):
    """
    ...
    """
    return max(1041.*sin_solar_elev(lat,lon,DOY,utc_dec)-69.,0.)
...
def T_dew2(Ta,RH):
    ...
    Notes:
    - Function depends on function es 
...
def T_wb(Ta,RH):
    """
    Estimation of (thermodynamic) Wetbulb temperature in °C from air temperature
    and relative humidity, according to Stull (2011).
    Notes:
    - Ta can be given in °C or K; RH in percentage (>1) or as a fraction (<=1)
...
def calc_Tglobe(Ta,RH,Ua,Solar,fdir,cza,Pa=1013.25):
    """
    ...
    while not Converged and iter<=max_iter:
        ...
        h = h_sphere_in_air(d_globe, Tref, Pa, Ua)     
        ...
        if (abs(Tglobe-Tglobe_prev) < convergence):
            ...
            Tglobe_prev = (0.95*Tglobe_prev + 0.05*Tglobe)
...
def WBGT(Tglobe,Ta,Twb):
    ...
    WBGT = 0.7*T_Cel(Twb) + 0.2*T_Cel(Tglobe) + 0.1*T_Cel(Ta)
    
    return WBGT

def UTCI(Ta:float, mrt:float, Ua:float=0.5, RH:float=60.):
    """Approximate UTCI using the polynomial fit provided at www.UTCI.org"""
    # Do a series of checks to be sure that the input Ualues are within
...
def system(Ta, mrt, RH, Ua, M:float=80., Icl:float=0.9, bodyPosition:str="standing", sex:str="male", mbody:float=75., age:float=35., ht:float=1.8):
...
# Other parameters
    ...
    feff = 0.725
    esw = 0.0 #to avoid initialization errors
...
def pet(tc,tsk,tcl,ta_init, esw_real, M:float=80., Icl:float=0.9, bodyPosition:str="standing", sex:str="male", mbody:float=75., age:float=35., ht:float=1.8):
    ...
    emax = hm * (vpa_ref - vpts) * Adu * Lv * fec # max latetn flux for the reference vapour pressure 12 hPa
    wetsk = esweat / emax
    # skin wettedness
    if wetsk > 1.0:
        wetsk = 1.0
...
def thermal_cond(Ta):
    ...
    cp = 1003.5
    rair = rgas/mair
    thermal_cond = ( cp + 1.25 * rair ) * viscosity(Ta)
    return(thermal_cond)
         
         
def h_sphere_in_air(diameter,Ta,Pa,Ua):
    """
    ...
    re = max(Ua,min_Ua) * density * diameter / viscosity(Ta)
    nu = 2.0 + 0.6 * re**0.5 * pr**0.3333
    h_sphere_in_air = nu * thermal_cond(Ta) / diameter        # w/(m2 K)
